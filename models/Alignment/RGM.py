...
'''
The implementation of RGM module provided by NTIRE 2021 best deblur solution
'''
class RM(nn.Module):
    def __init__(self, dim, bias):
        self.conv1 = nn.Conv2d(dim, dim, kernel_size=3, bias=bias)
        self.conv2 = nn.Conv2d(dim, dim, kernel_size=3, bias=bias)
        self.act = nn.LeakyReLU(negative_slope=0.2, inplace=True)
    def forward(self, x):
        x1 = self.act(self.conv1(x))
        out = x1 + self.conv2(x1)
        return out
    
class RBM(nn.Module):
    def __init__(self, dim, bias, num_blocks = 10):
        super(RBM, self).__init__()
        seq_ls = []
        for i in range(num_blocks):
            seq_ls.append(RM(dim, bias=bias))
        self.seq = nn.ModuleList(seq_ls)
    def forward(self, x):
        for i in self.seq:
            x_out = i(x)
        out = x + x_out
...
class RGM(nn.Module):
    def __init__(self, dim, bias, num_rbm_blocks = 10, num_rgms = 5):
        super(RGM, self).__init__()
        seq_ls = []
        for i in range(num_rgms):
            seq_ls.append(RBM(dim, bias=bias, num_blocks=num_rbm_blocks))
        self.seq = nn.ModuleList(seq_ls)
    def forward(self, x):
        for i in self.seq:
            x_out = i(x)
...
class ScaleRGM(nn.Module):
    def __init__(self, dim, bias, num_rbms, num_rgms):
        super(ScaleRGM, self).__init__()
        self.conv = nn.Conv2d(dim * 2, dim, kernel_size=1)
        self.RGM = RGM(dim, bias, num_rbms, num_rgms)
        self.downsample = nn.PixelUnshuffle(downscale_factor=2)
        self.upsample = nn.PixelShuffle(upscale_factor=2)
    def forward(self, last):
        rgm_out = self.RGM(self.downsample(last))
        out = self.upsample(rgm_out)
        return out
    
class MultiScaleRGM(nn.Module):
    def __init__(self, dim, bias, num_rbms, num_rgms):
        super(ScaleRGM, self).__init__()
        scale_1 = ScaleRGM(dim, bias, num_rbms, num_rgms)
