...
class DWTFunction_2D(Function):
    ...
    def backward(ctx, grad_LL, grad_LH, grad_HL, grad_HH):
        ...
        grad_L = torch.add(torch.matmul(grad_LL, matrix_Low_1.t().to(grad_LL.dtype)),
                           torch.matmul(grad_LH, matrix_High_1.t().to(grad_LL.dtype)))
        grad_H = torch.add(torch.matmul(grad_HL, matrix_Low_1.t().to(grad_LL.dtype)),
                           torch.matmul(grad_HH, matrix_High_1.t().to(grad_LL.dtype)))
        grad_input = torch.add(torch.matmul(
            matrix_Low_0.t().to(grad_LL.dtype), grad_L), torch.matmul(matrix_High_0.t().to(grad_LL.dtype), grad_H))
...
class IDWTFunction_2D(Function):
    ...
    def backward(ctx, grad_output):
        matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1 = ctx.saved_variables
        grad_L = torch.matmul(matrix_Low_0.to(grad_output.dtype), grad_output)
        grad_H = torch.matmul(matrix_High_0.to(grad_output.dtype), grad_output)
        grad_LL = torch.matmul(grad_L, matrix_Low_1.to(grad_output.dtype))
        grad_LH = torch.matmul(grad_L, matrix_High_1.to(grad_output.dtype))
        grad_HL = torch.matmul(grad_H, matrix_Low_1.to(grad_output.dtype))
        grad_HH = torch.matmul(grad_H, matrix_High_1.to(grad_output.dtype))
